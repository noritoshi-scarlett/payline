...
namespace Payline\Test\Infrastructure\Library\Cache;

use Payline\App\Infrastructure\Library\Cache\RedisCacheSystem;
...
class RedisCacheSystemTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->cacheSystem = new RedisCacheSystem($this->redisMock);
        $this->cacheSystem->prefixIdentification = 'testPrefix';
    }
    ...
    {
        $data = ['key1', 'key2'];
        $this->redisMock->method('get')->with('testPrefix##__ALL_KEYS__')->willReturn(serialize($data));
    ...
    {
        $keys = ['key1', 'key2'];
        $this->redisMock->method('set')->with('testPrefix##__ALL_KEYS__', serialize($keys))->willReturn(true);
    ...
    {
        $data = ['key' => 'value'];
        $this->redisMock->method('get')->with('testPrefix##testKey')->willReturn(serialize($data));
        $this->assertEquals($data, $this->cacheSystem->getByKey('testKey'));
    }

    public function testGetByKeyReturnsNullWhenKeyDoesNotExist(): void
    {
        $this->redisMock->method('get')->with('testPrefix##testKey')->willReturn(false);
    ...
    {
        $data = ['key' => 'value'];
        $this->redisMock->method('set')->with('testPrefix##testKey', serialize($data))->willReturn(true);
    ...
    {
        $data = ['key' => 'value'];
        $this->redisMock->method('set')->with('testPrefix##testKey', serialize($data))->willReturn(false);
