...
namespace Payline\Example\DatabaseImplementationExample;
...
readonly class LogRepository implements LogRepositoryInterface
{
    ...
    private function mapRowToEntity(mixed $row): LogEntityInterface
    {
        if (is_array($row)) {
            return new PaymentLog(
                $row['id'],
                $row['source_id'],
                $row['related_entity_colection_id'],
                PaymentLogEnum::from($row['state']),
    ...
    public function save(LogEntityInterface $log): bool
    {
        try {
            $query = sprintf(
                'INSERT INTO %s 
                    (source_id, related_entity_colection_id, state, data, created_at, message)
